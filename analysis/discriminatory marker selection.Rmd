...
pdf("../results/plots/decision_tree.pdf")
...
ENSG00000116882 (splits muscle and heart vs liver and kidney) = HAO2 = 2-Hydroxyacid oxidase 2
ENSG00000138356 (splits liver from kidney) = AOX1 = Aldehyde oxidase
ENSG00000130957 (splits heart from skeletal muscle) = FBP2 = Fructose-1,6-bisphosphatase isozyme 2
```{r}
...
discriminant_genes = c("ENSG00000116882", "ENSG00000138356", "ENSG00000130957")
...
for (i in discriminant_genes){
  ...
  ggsave(paste0("../results/plots/decision_tree_marker_", i, "_expression_v2.pdf"))
}
...
#ggsave("../results/plots/decision_tree_markers_heatmap.pdf")
```
...
<!-- discriminant_genes = c("ENSG00000021826", "ENSG00000113492", "ENSG00000130957") -->
...
# repeated, with a minimal set of genes
## Load and wrangle data
```{r}
log_tpm_tmm_goi = readRDS(file = "../results/log_tpm_tmm_goi.rds")
sample_meta = readRDS(file = "../results/sample_meta.rds")
...
selected_genes = read.csv("../data/20231009 Key enzymes metabolic pathways LL.csv") %>%
...
decision_trees_res %>%
  collect_predictions() %>%
  roc_curve(truth = tissue, .pred_Heart_Left_Ventricle, .pred_Kidney_Cortex, .pred_Liver, .pred_Muscle_Skeletal) %>%
  autoplot()

ggsave("../results/plots/decision_tree_roc.pdf")
```
...
#pdf("../results/plots/decision_tree2.pdf")
rpart.plot(decision_tree_fit$fit)
#dev.off()
...
rf_spec %>%
  set_engine("ranger", importance = "permutation") %>%
  fit(tissue ~ ., data = metabolic_genes_trees_juiced) %>%
  vip(geom = "point") +
  scale_y_continuous(expand = c(0, 0))
```
...
importance_scores <- rf_model$fit$variable.importance

# Convert to a data frame or tibble for a nice table format
importance_table <- tibble(
  variable = names(importance_scores),
  importance = importance_scores
)

# Optionally, arrange in descending order of importance
importance_table <- importance_table %>%
  arrange(desc(importance))

importance_table

top_vip = importance_table %>%
  top_n(10) %>%
  pull(variable)
...
for (i in top_vip){
  ...
  #ggsave(paste0("../results/plots/rf_marker_", i, "_expression.pdf"))
}
```
