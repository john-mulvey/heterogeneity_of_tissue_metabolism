...
log_tpm_grouped_tmm_goi  %>%
  ...
  pheatmap(scale = "row",
                   ...
                   annotation_row = pheat_inf_row,
           annotation_names_row = FALSE,
                   main = "Heatmap of metabolic gene expression (non-mito encoded) averaged per tissue, z-score")
```
...
de_cortex_vs_liver <- decideTestsDGE(et_cortex_vs_liver, adjust.method="BH", p.value=0.05)
...
results_aov = decideTestsDGE(qlf,adjust.method = "BH", p.value = 0.05, lfc = 2) #"p.value" is actually FDR
...
```{r}
#marleen genes only
log_tpm_tmm_goi[rownames(log_tpm_tmm_goi) %in% rownames(results_aov)[results_aov$adj.P.Val < 0.05],] %>%
pheatmap(
           scale = "none",
                   clustering_method = "ward.D2",
                   cluster_rows = T,
...
# all genes
log_tpm_tmm[rownames(log_tpm_tmm) %in% rownames(results_aov)[results_aov$adj.P.Val < 0.05],] %>%
pheatmap(cale = "none",
                  clustering_method = "ward.D2",
                   cluster_rows = T,
                   clustering_distance_rows = "euclidean",
...
equalizeLibSizes(y, dispersion=NULL, ...) #quantile normalisation between samples
pseudo.counts rather than cpm??? for heatmaps and PCA
