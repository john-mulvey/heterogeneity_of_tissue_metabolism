...
```{r}
keep <- filterByExpr(gene_reads_matrix, group = groups)
...
log_tpm_grouped_tmm  %>%
  rownames_to_column("ensembl_gene_id") %>%
  arrange(factor(ensembl_gene_id, levels = genes_of_interest$ensembl_gene_id)) %>%
  filter(ensembl_gene_id %in% ensg_mito_encoded$ensembl_gene_id) %>%
  column_to_rownames("ensembl_gene_id") %>%
  na.omit() %>%
  pheatmap(scale = "none",
                   clustering_method = "ward.D2",
                   ...
                   annotation_row = pheat_inf_row,
           annotation_names_row = FALSE,
                   main = "Heatmap of mito encoded gene expression averaged per tissue, z-score")
...
```{r}
#show individual samples
log_tpm_tmm_goi[rownames(log_tpm_tmm_goi) %in% rownames(limma_aov_results)[limma_aov_results$adj.P.Val < 0.05],] %>%
pheatmap(
           scale = "none",
                   clustering_method = "ward.D2",
                   cluster_rows = T,
