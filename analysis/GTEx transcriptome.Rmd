...
# gene_cpm_grouped_tmm = cpmByGroup(dgelist, log=FALSE, group = groups)
# gene_cpm_grouped_naive = cpmByGroup(dgelist, log=FALSE, group = groups)
# 
# #counts per million
# gene_cpm_tmm = cpm(dgelist, log = FALSE, group = groups)
```
...
<!-- # n_sv = num.sv(dgelist$counts, mod = design, method = "be") -->
<!-- # sv_obj = svaseq(dgelist$counts, mod = design, mod0 = design0, n.sv = n_sv) -->
<!-- # design_sv = cbind(design, sv_obj$sv) -->
<!-- #  -->
<!-- # # combat_matrix = ComBat_seq(counts = dgelist$counts, batch = sv_obj$sv, covar_mod = design) -->
<!-- # #  -->
<!-- # # combat_matrix = ComBat_seq(counts = dgelist$counts, batch = sv_obj$sv, covar_mod = design) -->
<!-- #  -->
...
<!-- # } -->
<!-- #  -->
<!-- # cleaned_counts = cleaningY(dgelist$counts, design_sv, P = 5) -->
...
<!-- ```{r} -->
<!-- # gene_length_goi = dgelist$counts %>% -->
<!-- #   as.data.frame() %>% -->
<!-- #   rownames_to_column("GTEx_ENSG") %>% -->
<!-- #   left_join(gene_info, by = "GTEx_ENSG") %>% -->
<!-- #   pull(gene_length) -->
<!-- #  -->
<!-- # #TMM -->
<!-- # rpkm_tmm_clean <- edgeR::rpkm(dgelist$counts, log=F, gene.length = gene_length_goi ) %>% -->
<!-- #   na.omit() -->
...
y = estimateDisp(dgelist, design, verbose = T)
...
edgeR_results_vs %>%
  ...
  filter(adj_PValue < pval_threshold  &
           (logFC.Heart_vs_others > logFC_threshold))
```
...
for (tissue1 in tissues) {
  for (tissue2 in tissues) {
    ...
    if (exists(edgeR_results)) {
      ...
      print(volano_plot_gene_of_interest)
  ...
  for (tissue2 in tissues) {
    ...
    if (exists(object_name)) {
      ...
      plot_code <- paste0("plot_obj <- ", object_name, "@result %>%
                          ...
                          ggtitle('", tissue1, " vs ", tissue2, ": enrichment of metabolic pathways')")
